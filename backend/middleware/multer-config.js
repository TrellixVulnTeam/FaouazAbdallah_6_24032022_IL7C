...
const storage = multer.diskStorage({
  ...
  filename: (req, file, callback) => { //2) filename pour indiquer quel nom de fichier utilisé
    const name = file.originalname.split(' ').join('_'); // on genere le nouveau nom du fichier ,split et join pour regler le probleme des espaces 
...
module.exports = multer({storage: storage}).single('image');//single pour indiquer que c'est un fichier unique et qu'il sagit de fichier image uniquement